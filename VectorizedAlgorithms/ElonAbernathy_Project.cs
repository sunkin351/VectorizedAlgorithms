...
namespace VectorizedAlgorithms
{
    ...
    */
    ...
    public class ElonAbernathy_Project
    {
        ...
        [Benchmark]
        public unsafe double VectorizedSolution()
        {
            if (!Sse41.IsSupported)
                throw new PlatformNotSupportedException();

            float[] results = new float[PointData.VectorElements * Vector128<float>.Count];
            ...
            for (int i = 0; i < PointData.VectorElements; ++i)
            {
                // float x = point.X;
                // float y = point.Y;
                // float z = point.Z;
                var point_X = PointData.GetXVector(i);
                var point_Y = PointData.GetYVector(i);
                var point_Z = PointData.GetZVector(i);
                ...
                for (int j = 0; j < vecSegments.Length; ++j)
                {
                    ...
                    */
                    ...
                    if (Fma.IsSupported)
                    {
                        ...
                        v11 = Fma.MultiplyAdd(v12, v12, v11);
    ...
    public struct VectorizedCalculationContext
    {
        ...
        {
            int revisedElementCount = RoundUp(elementCount, Vector128<float>.Count);

            X = new float[revisedElementCount];
            Y = new float[revisedElementCount];
            Z = new float[revisedElementCount];

            VectorElements = revisedElementCount / Vector128<float>.Count;
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Vector128<float> GetXVector(int vectorIndex)
        {
            if ((uint)vectorIndex >= (uint)this.VectorElements)
            {
                Throw_IndexOutOfRange();
            }

            return GetVector(X, vectorIndex);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Vector128<float> GetYVector(int vectorIndex)
        {
            if ((uint)vectorIndex >= (uint)this.VectorElements)
            {
                Throw_IndexOutOfRange();
            ...
            return GetVector(Y, vectorIndex);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Vector128<float> GetZVector(int vectorIndex)
        {
            if ((uint)vectorIndex >= (uint)this.VectorElements)
            {
                Throw_IndexOutOfRange();
            ...
            return GetVector(Z, vectorIndex);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static Vector128<float> GetVector(float[] arr, int vectorIndex)
        {
            return Unsafe.As<float, Vector128<float>>(ref Unsafe.Add(ref MemoryMarshal.GetArrayDataReference(arr), vectorIndex * Vector128<float>.Count));
        }
